...
* **Screenshot Reference:** `screenshots/01-ubuntu-apache-environment.png` *(Shows the active Ubuntu Linux terminal interface alongside the successful Apache2 default web server landing page).*
...
* **Screenshot Reference:** `screenshots/02-agents-departments-setup.png` *(Shows the organized list of departments, active agents, and their respective management roles).*
...
* **Screenshot Reference:** `screenshots/03-sla-policies-config.png` *(Shows the live configuration panel of the active SLA plans inside the system).*
...
* **Ticket Arrival:** The ticket safely landed in the general IT department's inbox with an assigned ticket ID.
  * **Screenshot Reference:** `screenshots/04-scenario1-ticket-received.png`
* **Investigation & Resolution:** The IT Agent (**Fahad**) picked up the ticket. After running local diagnostics, he determined it was an Active Directory issue. Fahad updated the user's security groups in the AD Domain Controller, verified the access fix, documented the internal technical logs, and successfully marked the ticket as resolved.
  * **Screenshot Reference:** `screenshots/05-scenario1-active-directory-resolution.png`
...
* **Initial Diagnostics:** The ticket was assigned to the general IT queue.
  * **Screenshot Reference:** `screenshots/06-scenario2-helpdesk-diagnostic.png` *(Shows the ticket thread where IT Agent Fahad investigated the workstation and left a technical note stating that local networking is fine, but perimeter Firewall rules are blocking the developer's external ports).*
...
* **Resolution by NOC:** The NOC Agent (**Rayan**) took ownership of the escalated ticket. He modified the enterprise Firewall policies to permit outbound developer traffic, confirmed the fix with the user, and closed the ticket.
  * **Screenshot Reference:** `screenshots/07-scenario2-noc-firewall-resolution.png` *(Shows the complete historical log of the ticket being transferred to NOC and successfully resolved).*
